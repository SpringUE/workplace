...
module.exports = async (browser, timeout, key) => {
  var getDataFromDom = async () => {
    ...
    await fs.appendFileSync(`./download/data/tianyancha-${key}.txt`, `startTime: ${new Date().toLocaleString()}`+'\r');
    await fs.appendFileSync(`./download/data/tianyancha-${key}.txt`, JSON.stringify(content, null , ' ')+'\r');
    await fs.appendFileSync(`./download/data/tianyancha-${key}.txt`, `endTime: ${new Date().toLocaleString()}`+'\r\r');
  }
